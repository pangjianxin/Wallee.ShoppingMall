...
export interface CallbackResult {
  ...
  username?: string;
  roles?: string;
  onSignIn?: (userData: UserData, user: any, account: any) => UserData | Promise<UserData>;
...
export function credentials<TSchema extends z.ZodSchema>(
  ...
  return {
    id: "credentials",
    endpoints: {
      signInCredentials: createAuthEndpoint(path, {
        method: "POST",
        ...
        if (!user) {
          ...
          let finalUserData: any = { ...userData };
          if (onSignIn) {
            const signInResult = await onSignIn({ email, ...userData }, user, account);
          ...
          if (finalUserData && Object.keys(finalUserData).length > 0) {
            ...
            user = await ctx.context.internalAdapter.updateUser(user.id, updateData);
          }
        }
