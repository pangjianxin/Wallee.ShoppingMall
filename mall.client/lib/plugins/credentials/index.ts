...
import type { CallbackResult } from "@/types/auth-types";
        ...
        async (ctx) => {
          ...
          const fallbackUsername =
            typeof userData.username === "string"
              ? userData.username
              : "username" in parsed && typeof (parsed as any).username === "string"
          ...
          if (!email && fallbackUsername) {
            email = `${fallbackUsername}@local.user`;
          }
          ...
          if (!user) {
            ...
            if (onLinkAccount) {
              ...
              const normalizedAccountData: Record<string, any> = {
                ...accountData,
              };
              if ("expiresAt" in normalizedAccountData) {
                normalizedAccountData.expiresAt = normalizeExpiresAt(
                  normalizedAccountData.expiresAt
                );
              }
            ...
            if (finalUserData && Object.keys(finalUserData).length > 0) {
              const updateData: Record<string, any> = { ...finalUserData };
          ...
          // 5. 创建 session
          let session = await ctx.context.internalAdapter.createSession(
            user.id,
            ctx.request as any
          );
          ...
          if (sessionFromCallback) {
            ...
            if ("expiresAt" in sessionUpdate) {
              sessionUpdate.expiresAt = normalizeExpiresAt(
                sessionUpdate.expiresAt
              );
            }
